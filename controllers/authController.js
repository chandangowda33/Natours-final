...
exports.logout = (req, res) => {
  //we set http only in login to make sure no one manipulate it in browser
  //but by this we can't delete cookie too so to workaround we are sending dummy cookie as 'loggedout
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    //first we only used to read from bearer but now we used cookies parser to parse the cookie will all the incoming request so nwe can use token in all the requests and use it
...
exports.isLoggedIn = async (req, res, next) => {
  console.log(`Coo:${req.cookies.jwt}`);
  if (req.cookies.jwt) {
    try {
      // 1) verify token
